# Research & Learn


## What are examples of professional vs. unprofessional behaviour in a casual work environment? (Hint: Think about communication, meeting etiquette, and teamwork.)

## Professional vs Unprofessional Behaviour

### Professional Behaviour:
- Communicating clearly and respectfully  
- Being punctual and prepared for meetings  
- Supporting teammates and collaborating effectively  
- Accepting feedback positively  

### Unprofessional Behaviour:
- Ignoring messages or being unresponsive  
- Using disrespectful or unclear language  
- Not taking responsibility for tasks  
- Reacting negatively to feedback  



## What does respectful communication look like in remote teams? (e.g., tone in messages, handling disagreements, responding to feedback)

In remote teams, communication should be:
- Clear and direct to avoid confusion  
- Polite and respectful in tone  
- Timely, so others are not blocked  

When disagreements happen, I will focus on understanding the issue rather than reacting emotionally, and aim to resolve it constructively.

## How can you give and receive feedback professionally? (e.g., focus on actions, not people; assume good intent; ask for clarification instead of reacting defensively)

When giving feedback:
- Focus on the task, not the person  
- Be constructive and specific  

When receiving feedback:
- Stay open-minded and not defensive  
- Ask questions if something is unclear  
- Use it as an opportunity to improve  

i have reviewed as part of research 

# Reflection


## Have you ever experienced or witnessed great teamwork and professionalism? What made it effective?

In my previous experiences, good teamwork happened when communication was clear, responsibilities were well-defined, and everyone respected each other’s ideas. This created a positive and productive environment.

## What steps can you take to make sure your communication is clear, respectful, and inclusive?

To ensure my communication is clear and inclusive, I will:
- Use simple and understandable language  
- Avoid assumptions  
- Be respectful and patient in conversations  

## How can you help create a positive, fun, and professional work environment?

I can contribute by:
- Being supportive and approachable  
- Respecting different perspectives  
- Maintaining a positive attitude even during challenges  

## How can you prepare effectively for meetings with supervisors and colleagues?

To prepare effectively, I will:
- Review the agenda beforehand  
- Note down key points or questions  
- Stay attentive and participate actively 

## What would it look like to be proactive during your internship?

Being proactive means:
- Taking initiative instead of waiting for instructions  
- Asking questions when unsure  
- Following up on tasks and responsibilities  

## How might you follow up with colleagues or supervisors if you need information/action from them? How would you escalate it further if you don't get a response and when would you escalate?

If I need a response:
- I will send a polite follow-up message after a reasonable time  
- If there is still no response, I will escalate respectfully to the relevant person or supervisor  

Escalation will only be done when necessary, such as when work is blocked or deadlines are affected.


## What are some things you won't do?

- Ignoring messages or delaying responses unnecessarily  
- Using unclear or disrespectful communication  
- Avoiding responsibility for my work  
- Reacting negatively to feedback 

- I understand that if I notice any serious issues such as inappropriate behaviour or breaches of professional conduct, I should report them responsibly using the appropriate channels, including contacting leadership if required.
- And i also noted CEO 's phone number from onboarding deck 
